...
| MVP-02 | Content repo structure (`content/subjects.json`, `content/prompt_templates/`, `content/questions/`) + 50 hand-written text Qs across 3 subjects | — | Done (`a352ab5`) | — |
| MVP-03 | `@quiz/sdk` — Zod schemas (Question, Pack, Manifest, GA4 events), grading helper, `QuestionCard` template renderer for RN, analytics abstraction (console logger in Phase 0) | — | Done (`7fdc174`) | — |
...
**Note:** Shipped tasks (MVP-01–03) carry `—` in Effort and Blocked By per plan §6 (no retroactive task-spec rewrites).
